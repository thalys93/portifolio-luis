...
function AdminProject() {
    ...
    const NewProjectValues: Partial<projectsInterface> = {
        ...
        details: {
            bigDescription: projectData?.details.bigDescription,
            technologies: projectData?.details.technologies,
        },
        status: projectData?.status,
        type: projectData?.type,
        projectLive: projectData?.projectLive,
    ...
    async function handleSaveProject(values: Partial<projectsInterface>) {
        ...
        const mappedValues: Partial<projectsInterface> = {            
            ...
            status: values.status || projectData?.status,
            type: values.type || projectData?.type,
            projectLive: values.projectLive || projectData?.projectLive,
            projectRepository: values.projectRepository || projectData?.projectRepository,
            image: values.image || projectData?.image,
            conceptArt: values.conceptArt || projectData?.conceptArt,
        }
            ...
            <Row>
                <section>
                    <article className='mt-3 lg:m-5 select-none'>
                        <h1 className='text-xl animate__animated animate__fadeIn animate__slower'> Editor de Projeto </h1>
                        <span className='text-stone-500 opacity-30 animate__animated animate__fadeIn animate__slower'>Altere as informações do seu projeto</span>
                                ...
                                <section className='flex flex-row items-center justify-between mt-3'>
                                    <article className='flex flex-row gap-3'>
                                        ...
                                        <div>
                                            ...
                                            <Input type='project-repository' onChange={handleChange('projectRepository')} onBlur={handleBlur('projectRepository')} value={values.projectRepository as string || projectData?.projectRepository as string} />
